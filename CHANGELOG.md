...
- CI: GitHub action runs [zizmor] with regular persona.
...
- Upgrade `click` for [PYSEC-2026-2132], except `covimerage` because
  (vader group, dev-only Vim coverage tool) pins `click<7.1` and
...
- Issue {issue}`6`, suppress `SyntaxWarning` from vim_bridge and panflute
  appearing in Vim.
- `PytestRemovedIn10Warning`: `pytest` deprecated passing a non-Collection
  iterable to pytest.parametrize().
- Profile Vader tests during PyTest with default tox `[testenv]`.
- `tox -e vim` runs `covimerage` on existing `tmp/vim-profile.txt`,
  generated by PyTest.
- Separate `covimerage` configuration from `coverage.py`,
  because `pytest-cov` requires a later version than `covimerage` supports.
- [kdheepak/panvimdoc v4.0.1] does not parse `yaml`
  header in `README.md`.

### Changed

- Expected test results per [pandoc v3.10].
...
- Issue {issue}`2`. Change `delete_taskwiki_heading` to remove anchors and work
  for markdown and vimwiki syntax.
- `tox -e linkcheck` errors.
- v0.10.0, fix incorrect version of `pypa/gh-action-pypi-publish`.

### Changed

- `VimwikiConvert` supports Vimwiki Markdown syntax (Issue {issue}`2`).
